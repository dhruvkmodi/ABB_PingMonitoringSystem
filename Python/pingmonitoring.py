...
def delete_content_of_ipoutput():
        ...
        f = open("./Python/ipoutput.txt", "w") # open output file with write permissions ["w" mean overwrite the file so the old content of the file will be deleted and replaced by the new content]
...
def monitoring_ping(OStype): #OStype parameter function will be explained later in the code
        ...
        with open("./Python/ip_list.txt") as file: #open the input file
        ...
        for ip in dump: #for loop to go through each IP address in the list, find out the ping status and store the result in the output file 
                ...
                if response == 0: #if the response type is equal to zero [reponse is equal to zero means that the ping is successful]
                        ...
                        if "Destination host unreachable" in DHU: # check if the string/text "Destination host unreachable" is part of the "DHU" variable 
                                f = open("./Python/ipoutput.txt", "a") #open the output file and append ["a" means add to the end of file without overwriting the file so the content that is already part of the file will stay and the new cotent will be added to the end of the file]
                        ...
                        f = open("./Python/ipoutput.txt", "a") #open the output file with the append parameter 
