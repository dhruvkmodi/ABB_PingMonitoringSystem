$iplist = Get-Content 'C:\Users\800xaInstaller\OneDrive - ABB\Documents\Ping Monitoring\PowerShell\ip_list.txt' #Get the IP Addresses[content of the file] from the Input File and store in in the variable "$iplist"
Remove-Item 'C:\Users\800xaInstaller\OneDrive - ABB\Documents\Ping Monitoring\PowerShell\ipoutput.txt' #Delete the Old Output File
...
foreach($ip in $iplist){ #For Loop, this will allow us to find the ping status of the IP Addresses independently
    ...
    #-ComputerName $ip: States the IP Address which in this case is stored in the variable "$ip"
    #-Quiet: Returns the boolean [$True if Ping is Successful and $False if Ping is not Successful]
    #-Count 1: Will ping the IP Address only once
    #-ErrorAction SilentlyContinue: If the pinging is not successful, then instead of returning errors, it will return that the IP Address is offline as the output
